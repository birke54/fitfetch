 ...
 *
 * <p>The bean is injected into the ATS fetchers (for example
 * {@link GreenhouseFetch}) and the geocoder. Every client built here carries a
 * connect and a read timeout. The JDK client underneath has neither by default,
 * so one hung connection would otherwise stall a scheduled run indefinitely.
 */
@Configuration
